...
public class dollsempire {
    public static void main(String[] args) throws IOException {
        ...
        String Tovar = "Фешн куклы Groove (Пуллип) - Pullip";
...
///////////////////уровень категории
String PathCategory = "https://dollsempire.ru/pullip_heroes_dolls.html";
        String Zag2 = "Персонажи Пуллип (Pullip)";

        Document docCategory = Jsoup.connect(PathCategory).get();

        Elements linksCategory = docCategory.getElementsByClass("subcat-link2");
        int CountCategory=0;

            for (Element linksCategorys : linksCategory) {

                String NameCategory = docCategory.getElementsByClass("subcat-link2").text();
                System.out.println(NameCategory);
//                if(NameCategory.equals(Zag2)) {
                String addressCategory = linksCategory.get(CountCategory).select("a[href]").attr("abs:href");
//                String CategoryAutoName = docCategory.getElementsByTag("h1").text();
